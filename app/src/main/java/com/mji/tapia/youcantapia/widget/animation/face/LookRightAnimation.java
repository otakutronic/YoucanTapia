...
public class LookRightAnimation extends Animation {

    public enum LookRightEvent implements Event {
        LOOK_RIGHT_EYES_RIGHT,
        LOOK_RIGHT_EYES_BACK_START,
        LOOK_RIGHT_ANIM_FINISH
    }
    ...
    int lookRightFrameCount;
    int lookRightFrameCountDown;
    ...
    public LookRightAnimation(Context context){
        ...
        try {
            ...
            lookRightFrameCountDown = 0;
            lookRightFrameCount = 40;
    ...
    protected void update(long frameSkip) {
        if (lookRightFrameCountDown > 0) {
            lookRightFrameCountDown -= frameSkip;
            if (lookRightFrameCountDown <= 0) {
                backIrisFrameIndex = 0;
                if(animationListener != null) {
                    animationListener.onEvent(LookRightAnimation.this, LookRightEvent.LOOK_RIGHT_EYES_BACK_START);
                }
            }
        } else if (startIrisFrameIndex < startIrisFrameCount) {
            startIrisFrameIndex += frameSkip;
//            leftIrisSpeedX = ;
            leftIrisPosX = leftInitIrisPosX + (int) (28 * (Math.cos((startIrisFrameIndex / (float) startIrisFrameCount + 1.f) * Math.PI) / 2.f + 0.5f));
            leftIrisSpeedX = (int) (-28 * Math.PI * Math.sin(Math.PI * (startIrisFrameIndex / (float) startIrisFrameCount + 1.f)) / 2.f);
            rightIrisPosX = rightInitIrisPosX + (int) (288 * (Math.cos((startIrisFrameIndex / (float) startIrisFrameCount + 1.f) * Math.PI) / 2.f + 0.5f));
            rightIrisSpeedX = (int) (-288 * Math.PI * Math.sin(Math.PI * (startIrisFrameIndex / (float) startIrisFrameCount + 1.f)) / 2.f);
            if (startIrisFrameIndex >= startIrisFrameCount) {
                lookRightFrameCountDown = lookRightFrameCount;
                leftIrisPosX = leftInitIrisPosX + 28;
                rightIrisPosX = rightInitIrisPosX + 288;
                leftIrisSpeedX = 0;
                rightIrisSpeedX = 0;
                if(animationListener != null) {
                    animationListener.onEvent(LookRightAnimation.this, LookRightEvent.LOOK_RIGHT_EYES_RIGHT);
                }
            }
        } else if (backIrisFrameIndex < backIrisFrameCount) {
            backIrisFrameIndex += frameSkip;

            leftIrisPosX = leftInitIrisPosX + 28 + (int) -(28 * (Math.cos((backIrisFrameIndex / (float) backIrisFrameCount + 1.f) * Math.PI) / 2.f + 0.5f));
            leftIrisSpeedX = (int) (28 * Math.PI * Math.sin(Math.PI * (backIrisFrameIndex / (float) backIrisFrameCount + 1.f)) / 2.f);
            rightIrisPosX = rightInitIrisPosX + 288 - (int) (288 * (Math.cos((backIrisFrameIndex / (float) backIrisFrameCount + 1.f) * Math.PI) / 2.f + 0.5f));
            ...
            if (backIrisFrameIndex >= backIrisFrameCount) {
                ...
                rightIrisSpeedX = 0;
                if(animationListener != null) {
                    animationListener.onEvent(LookRightAnimation.this, LookRightEvent.LOOK_RIGHT_ANIM_FINISH);
                }
            }
        }
